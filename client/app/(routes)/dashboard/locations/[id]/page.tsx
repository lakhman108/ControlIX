...
import { ArrowLeft, Power, Settings2 } from "lucide-react";
    ...
    const fetchLocation = async () => {
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_BACKEND_URL}location/${params.id}`,
          { credentials: "include" }
        );
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div
                        ...
                        onClick={() =>
                          router.push(`/dashboard/devices/${device.id}?code=${device?.deviceType}`)
                        }
                      >
